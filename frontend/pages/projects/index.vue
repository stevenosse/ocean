<template>
  <div>
    ...
    <!-- Projects List View -->
    <div v-else-if="viewMode === 'list' && filteredProjects.length > 0" class="bg-white shadow overflow-hidden sm:rounded-lg">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="project in filteredProjects" :key="project.id">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 space-x-2">
              ...
              <NuxtLink :to="`/projects/${project.id}/edit`" class="text-indigo-600 hover:text-indigo-900">Edit</NuxtLink>
              <button 
                v-if="project.active"
                @click="triggerDeploy(project.id)" 
...
const projectDeployments = ref<Record<number, Deployment>>({}) // Store latest deployment for each project
...
// Fetch projects and their latest deployments
onMounted(async () => {
  try {
        ...
        fetchedProjects.map(async (project) => {
          try {
            ...
            if (deployments && deployments.length > 0) {
              // Sort deployments by date (newest first) and get the latest one
...
const triggerDeploy = async (projectId: number) => {
  try {
    ...
    if (result) {
      ...
      // Update the project deployments with the new deployment
